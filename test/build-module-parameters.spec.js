...
describe('build module parameters', () => {
  it('should return param if exists', () => {
    buildModuleParameters({
      parameters: [{ in: 'body', name: 'val' }],
    },
      {
        method: 'GET',
        parameters: {
  ...
  it('should return multiple parameters if exists', () => {
    buildModuleParameters({
      parameters: [{ in: 'body', name: 'val' }, { in: 'headers', name: 'authorization' }],
    },
      {
        method: 'GET',
        parameters: {
      ...
      }).parameters.should.eql(['test', 123]);
  });

  it('should return all objects if wildcard', () => {
    buildModuleParameters({ parameters: [{ in: 'headers', name: '*' }] }, {
      method: 'GET',
      parameters: {
        headers: {
          test: true,
    ...
    }).parameters.should.eql([{ test: true, value: 'asd' }]);
  });

  it('should return empty object if not exist and optional', () => {
    buildModuleParameters({
      parameters: [{ in: 'body', name: 'test', required: false }],
    },
      {
        method: 'GET',
        parameters: {},
